...
│    • RescueAlly(Ally)          - Extract wounded teammate       │
...
**Option A: AI System First → Combat System Later**
```
Weeks 1-14: AI Refactoring (Value Network, World Model, MCTS coupling)
               ↓
            AI learns old action space (16 tactical actions)
               ↓
Weeks 15-20: Combat Refactoring (Objectives, atomic actions)
               ↓
            ❌ AI must be retrained from scratch
            ❌ World model predictions invalid (old action space)
            ❌ Value network assumptions broken
            ❌ MCTS tree structure obsolete
               ↓
Weeks 21-24: Redo AI training with new combat system
               ↓
Total: 24 weeks
```

**Option B: Combat System First → AI System Later** ✅ **RECOMMENDED**
```
Weeks 1-6: Combat Refactoring (Objectives, atomic actions, hierarchical rewards)
             ↓
          Foundation stable, action space finalized
             ↓
Weeks 7-20: AI Refactoring (Value Network, World Model, MCTS coupling)
             ↓
          ✅ Train on final action space (8-dim atomic)
          ✅ World model predicts stable action format
          ✅ Value network learns objective values
          ✅ No rework needed
             ↓
Total: 20 weeks (4 weeks faster + less risk)
```
...
#### 1. Combat System is the Foundation
**The AI system learns from combat actions**
- Value Network evaluates states → Needs to know what actions exist
- World Model predicts next state → Needs to know action effects
- MCTS explores action space → Needs stable action definitions
- RL Policy outputs actions → Needs final action format

**Changing actions mid-way = expensive rework**
- If we train Value Network on 16 tactical actions, then switch to 8-dim atomic actions:
  - Value Network inputs invalid (observation includes action context)
  - World Model predictions invalid (predicts wrong action space)
  - MCTS tree structure invalid (explores wrong action combinations)
  - Need to retrain from scratch (~2-3 weeks lost)

#### 2. Combat Refactoring Enables AI Refactoring
**Key dependencies:**

| AI Component | Requires from Combat System |
|--------------|----------------------------|
| **Value Network** | Stable objective definitions (what constitutes "win"?) |
| **World Model** | Predictable atomic actions (move, fire, crouch) |
| **MCTS Action Space** | Small, meaningful objective set (not 14,641 combinations) |
| **Hierarchical Rewards** | Clear objective progress signals |
| **Policy Priors** | Objective context (what is agent trying to do?) |

**Current combat system blocks AI refactoring:**
- 16 tactical actions → Hard to predict (world model struggles)
- 14,641 MCTS combinations → Too many to search efficiently
- Misaligned rewards → Value network can't learn objective values
- Ambiguous commands → No clear objective context for policy

**Refactored combat system unblocks AI:**
- 8-dim atomic actions → Easy to predict (continuous regression)
- ~50 objective assignments → MCTS searches 300x smaller space
- Hierarchical rewards → Value network learns objective importance
- Explicit objectives → RL policy conditions on clear intent

#### 3. Combat Refactoring is Self-Contained
**Can be completed independently:**
- No dependencies on Value Network, World Model, or MCTS changes
- Works with existing rule-based MCTS (v2.0)
- Works with existing RL policy (just changes action format)
- Can validate immediately (manual testing, existing scenarios)

**Incremental validation:**
- Sprint 1: Objective system works
- Sprint 2: Atomic actions execute correctly
- Sprint 3: StateTree simplified
- Sprint 4: Rewards calculated properly
- Sprint 5: MCTS selects objectives
- Sprint 6: End-to-end validation

Each sprint delivers working functionality.

#### 4. Risk Mitigation
**Combat First = Lower Risk**
- Combat changes visible immediately (visual debug, logs)
- Errors caught early (agents don't move correctly)
- Can fallback to v2.0 if needed (keep old system parallel)
- No ML training dependency (can test with random policy)

**AI First = Higher Risk**
- ML training results delayed (need hundreds of episodes)
- Errors hidden in training curves (overfitting? bad rewards?)
- Can't validate until training converges (~1 week per experiment)
- If combat changes later, lose all training progress

#### 5. Development Velocity
**Combat First:**
```
Week 1-6:   Combat refactoring (6 developers)
Week 7-20:  AI refactoring (6 developers) + Content creation (gameplay using new objectives)
               ↓
            Gameplay team can create scenarios while AI team works
            Parallelized work, higher throughput
```

**AI First:**
```
Week 1-14:  AI refactoring (6 developers, content team blocked)
Week 15-20: Combat refactoring (6 developers, AI team blocked)
Week 21-24: Redo AI training (6 developers, content team still blocked)
               ↓
            Sequential work, lower throughput
```
...
### ✅ Refactor Combat System First (Weeks 1-6)

**Pros:**
- Stable foundation for AI refactoring
- No rework needed later
- Can validate immediately
- Enables parallel content development
- Lower risk (visible bugs)
- Industry best practice

**Cons:**
- 6 weeks before starting AI work (but saves 4 weeks total)

### ❌ Refactor AI System First (NOT Recommended)

**Pros:**
- Start "cool ML stuff" immediately

**Cons:**
- Must retrain from scratch after combat changes
- 4 weeks lost to rework
- Higher risk (hidden ML bugs)
- Blocks content development
