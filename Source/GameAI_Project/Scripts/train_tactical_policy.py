...
class PPOTrainer:
    ...
    def export_onnx(self, output_path):
        ...
        # Create a wrapper module that only outputs action probabilities
        ...
        wrapper = PolicyInferenceWrapper(self.policy).to(self.device)
        wrapper.eval()

        # Create dummy input
        dummy_input = torch.randn(1, self.config.INPUT_SIZE).to(self.device)

        # Export with both outputs for compatibility
        torch.onnx.export(
            wrapper,
            dummy_input,
            output_path,
        ...
        print(f"  Input: observation [batch_size, 71]")
        print(f"  Output: action_probabilities [batch_size, 16] (softmax applied)")
        print(f"  Output: state_value [batch_size, 1]")
